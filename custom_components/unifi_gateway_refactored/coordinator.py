...
class UniFiGatewayDataUpdateCoordinator(DataUpdateCoordinator[UniFiGatewayData]):
    ...
    def _fetch_data(self) -> UniFiGatewayData:
        ...
        ui_host_wans: List[Dict[str, Any]] = []
        try:
            ui_host_wans = self._client.get_ui_host_wan_links()
        except Exception as err:  # pragma: no cover - defensive logging
            _LOGGER.debug("Failed to fetch WAN details from api.ui.com: %s", err)
        if ui_host_wans:
            _LOGGER.debug(
                "Augmenting WAN links with %s api.ui.com host records",
                len(ui_host_wans),
            )
