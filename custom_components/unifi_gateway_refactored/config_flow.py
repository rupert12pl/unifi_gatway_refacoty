...
class ConfigFlow(config_entries.ConfigFlow):
    ...
    async def async_step_user(
        ...
        return self.async_show_form(
            step_id="user", data_schema=basic_schema, errors=errors
        )
        ...
        adv_schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_USE_PROXY_PREFIX, default=DEFAULT_USE_PROXY_PREFIX
                ): bool,
                vol.Optional(CONF_TIMEOUT, default=DEFAULT_TIMEOUT): int,
            }
        )
        return self.async_show_form(
            step_id="advanced", data_schema=adv_schema, errors=errors
        )
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_USERNAME, default=self._entry.data.get(CONF_USERNAME)
                ): str,
                vol.Optional(
                    CONF_PASSWORD, default=self._entry.data.get(CONF_PASSWORD)
                ): str,
                vol.Optional(
                    CONF_SITE_ID,
                    default=self._entry.data.get(CONF_SITE_ID, DEFAULT_SITE),
                ): str,
                vol.Optional(
                    CONF_VERIFY_SSL,
                    default=self._entry.data.get(CONF_VERIFY_SSL, DEFAULT_VERIFY_SSL),
                ): bool,
                vol.Optional(
                    CONF_USE_PROXY_PREFIX,
                ...
                vol.Optional(
                    CONF_TIMEOUT,
                    default=self._entry.data.get(CONF_TIMEOUT, DEFAULT_TIMEOUT),
                ): int,
                vol.Optional(
                    CONF_VPN_FAMILY_OVERRIDE,
                    default=self._entry.options.get(
                        CONF_VPN_FAMILY_OVERRIDE, VPN_FAMILY_AUTO
                    ),
                ): vol.In(
